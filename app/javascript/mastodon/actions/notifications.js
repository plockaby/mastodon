...
export const fetchNotificationRequests = () => (dispatch, getState) => {
  if (!getState().getIn(['notificationRequests', 'items']).isEmpty() || getState().getIn(['notificationRequests', 'isLoading'])) {
    return;
  }

  dispatch(fetchNotificationRequestsRequest());

  api(getState).get('/api/v1/notifications/requests').then(response => {
    const next = getLinks(response).refs.find(link => link.rel === 'next');
    importFetchedAccounts(response.data.map(x => x.account));
...
export const fetchNotificationsForRequest = accountId => (dispatch, getState) => {
  if (!getState().getIn(['notificationRequests', 'current', 'notifications', 'items']).isEmpty() || getState().getIn(['notificationRequests', 'current', 'notifications', 'isLoading'])) {
    return;
  }

  dispatch(fetchNotificationsForRequestRequest());

  api(getState).get('/api/v1/notifications', { params: { account_id: accountId } }).then(response => {
    const next = getLinks(response).refs.find(link => link.rel === 'next');
    dispatch(importFetchedStatuses(response.data.map(item => item.status).filter(status => !!status)));
